...
namespace PCSC
{
    public class PCSCReader : IDisposable
    {
        ...
        public string TransmitExplicit(byte[]  explicitCommand)
        {
            if (explicitCommand == null || explicitCommand.Length == 0)
            {
                throw new Exception("No se puede ejecutar un comando nulo o sin elementos en el arrglo de byte");
            }
            byte[] recvBuffer = new byte[256];
            int recvLength = recvBuffer.Length;
            IntPtr sendPci = IntPtr.Zero;
            ...
            if (outputDebugString)
            {
                ...
                foreach (byte b in explicitCommand)
                {
                    sb.AppendFormat("{0:X2}", b);
                }
            ...
            int result = WinSCard.SCardTransmit(card, sendPci, explicitCommand, explicitCommand.Length, IntPtr.Zero, recvBuffer, ref recvLength);
            if (result != WinSCard.SCARD_S_SUCCESS)
            {
                throw new PCSCException(result);
            }
            ...
            byte[] response = new byte[recvLength];
            Buffer.BlockCopy(recvBuffer, 0, response, 0, recvLength);
            ///TODO data del usuario
            var data = Encoding.ASCII.GetString(response);
            return data;
        }

        public byte[] transmit(byte[] explicitCommand)
        {
            if (explicitCommand == null || explicitCommand.Length == 0)
            {
                throw new Exception("No se puede ejecutar un comando nulo o sin elementos en el arrglo de byte");
